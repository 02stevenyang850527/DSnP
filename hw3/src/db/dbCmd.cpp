		...
		for (int i = 1; i < len + 1; i++){
			...
			if (i < int(options.size())){
				if (myStr2Int(options[i], num))
					...
					store.push_back(INT_MAX);
				else 
     			{	cerr << "Error: " << options[i] << " is not a number!!\n"; return CMD_EXEC_DONE;}
			}
			else store.push_back(INT_MAX);
		}
		DBRow temp(store);
		dbtbl.addRow(temp);
	}
   ...
   // TODO...
	...
	if (options.size() == 2){
		if (myStrNCmp("-Row", options[0], 2) == 0){
			...
			{	cout << dbtbl[c] << endl; return CMD_EXEC_DONE;}
			else return CMD_EXEC_DONE;
		}
		else if (myStrNCmp("-Column", options[0], 2) == 0){
			int c;
			if (checkColIdx(options[1], c))
			{	dbtbl.printCol(c); cout << endl; return CMD_EXEC_DONE;}
			else return CMD_EXEC_DONE;
		}
		else{
			if (checkRowIdx(options[0], a) && checkColIdx(options[1], b))
				{dbtbl.printData(cout, dbtbl.getData(a, b), true); cout << endl; return CMD_EXEC_DONE;}
			else return CMD_EXEC_DONE;
		}
		return CmdExec::errorOption(CMD_OPT_ILLEGAL, options[0]);
	}
